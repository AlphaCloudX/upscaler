...
augmentSavePath = r"D:\upscaler_v2\data\val\lowRes"

trainDataDir = "data/val"
...
def loadData(direc, output):
    ...
    for i in os.listdir(direc):
        ...
        print(f"Done: {count} / {len(os.listdir(direc))}")
...
def randomNoise(img, noise_typ):
    ...
    noise = tf.random_normal(shape=shape, mean=0.0, stddev=1, dtype=tf.float32)
    return img + noise
...
for i in range(len(highRes)):
    ...
    for j in range(0, 2):
        ...
        # if random.randint(0, 2) == 0:
        #     img = randomNoise(img, random.randint(0, 5))
    ...
    if saveAugmentedTrainingImages:
        ...
        if not isinstance(img, numpy.ndarray):
            array = img.numpy()
            Image.fromarray(array.astype(np.uint8)).save(augmentSavePath + fr"\{i}.jpeg")

        else:
            Image.fromarray(img.astype(np.uint8)).save(augmentSavePath + fr"\{i}.jpeg")
